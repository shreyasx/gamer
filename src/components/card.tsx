    ...
    >
      ...
      <div className="flex flex-row rounded-b-3xl w-full">
        <div className="w-full flex flex-col">
          <p className="text-light2 text-center flex justify-center items-center text-5xl px-2 py-3">
            {title}
          </p>
          <div className="flex flex-row w-full h-full justify-center flex-wrap gap-3">
            {tags.map((tag) => (
              <span
                key={tag.id}
                className={`text-navy bg-light2 text-center flex justify-center items-center text-lg rounded-full mx-1 px-4 py-1`}
              >
                {tag.title}
              </span>
            ))}
